  ...
  .config(['$routeProvider',
    function($routeProvider) {
      $routeProvider
        ...
        .when('/update-phongban/:id', {
          templateUrl: 'pages/phong_ban/update_phong_ban.html',
          controller: 'update_phongbanCtrl'
        })
        .otherwise({
          redirectTo: '/'
        });
...
app.controller("phongbanCtrl",function($scope,$http){
  $http.get("http://localhost/laravel/public/api/phongban").then(function(response){
  ...
  $scope.xoa = function(i){
    // $scope.list_phongban.splice($scope.i,1);
    $http.delete("http://localhost/laravel/public/api/phongban/"+$scope.i).then(function(response){
  ...
  // $scope.edit = function(){
  //   $http.get("http://localhost/laravel/public/api/phongban/6").then(function(response){Ư
  //   $scope.phongbanId = response.data;
    
  //   });
  // }
});
});
app.controller("update_phongbanCtrl",function($scope,$http){
  // get theo id
  $http.get("http://localhost/laravel/public/api/phongban/6").then(function(response){
  $scope.phongbanId = response.data;
  });

  // update
  $scope.update_phongban =  function(){
    $http.put('http://localhost:3000/loaiphongban'+$scope.id_phongban,{
        name: $scope.name
...
app.controller("AddphongbanCtrl",function($scope,$http){
  $scope.add_phongban =  function(){
      $http.post('http://localhost/laravel/public/api/phongban',{
        name_phongban: $scope.name_phongban
      }).then(function(response){
